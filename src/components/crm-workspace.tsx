...
import {ChevronDown,Edit3,Mail,MessageCircle,Phone,Plus,Search,UserRound,X} from "lucide-react";
...
function ClientForm({client,onClose}:{client?:CrmClient;onClose:()=>void}){
 const router=useRouter();
 const[state,action,pending]=useActionState(saveCrmClient,initialState);
...
function ClientDrawer({drawer,onClose,onEdit}:{drawer:Drawer;onClose:()=>void;onEdit:(client:CrmClient)=>void}){
 ...
 return <><button className="drawer-backdrop" aria-label="Fechar painel do cliente" onClick={onClose}/><aside className="crm-client-drawer" role="dialog" aria-modal="true" aria-labelledby="crm-client-drawer-title"><header className="crm-client-drawer-header"><div><small>{drawer.mode==="view"?"Ficha do cliente":drawer.mode==="edit"?"Editar cliente":"Novo cliente"}</small><h2 id="crm-client-drawer-title">{drawer.mode==="new"?"Novo cliente":client?.name}</h2><p>{drawer.mode==="view"?"Contato, preferências e relacionamento em um só lugar.":"Cadastre as informações básicas e complemente quando quiser."}</p></div><button type="button" aria-label="Fechar" onClick={onClose}><X/></button></header>{drawer.mode==="view"&&client?<ClientSummary client={client} onEdit={()=>onEdit(client)}/>:<ClientForm key={`${drawer.mode}-${client?.id||"new"}`} client={client} onClose={onClose}/>}</aside></>;
}
...
export function CrmWorkspace({userId,query,view,columns,cards,clients,dealTotal,clientTotal,selectedClientId,openNew}:{userId:string;query:Query;view:"funil"|"clientes";columns:Column[];cards:DealCard[];clients:CrmClient[];dealTotal:number;clientTotal:number;selectedClientId?:string;openNew?:boolean}){
 ...
 return <div className="admin-content crm-center">
  ...
  <Pagination query={{...query,view}} page={Number(query.page)||1} total={view==="funil"?dealTotal:clientTotal}/>
