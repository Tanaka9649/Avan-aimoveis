...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [p] = await db.select().from(properties).where(user ? eq(properties.id, id) : and(eq(properties.id, id), eq(properties.status, "disponivel"), isNotNull(properties.publishedAt))).limit(1);
  if (!p) return new NextResponse(null, { status: 404 });
  const [photo] = await db.select({ path: propertyPhotos.storagePath }).from(propertyPhotos).where(and(eq(propertyPhotos.propertyId, id), eq(propertyPhotos.isCover, true))).limit(1);
