...
export default async function PropertiesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const [properties, params] = await Promise.all([publicProperties(), searchParams]);
  const initial = Object.fromEntries(["q", "type", "bedrooms"].map((key) => [key, typeof params[key] === "string" ? params[key] : ""]));
