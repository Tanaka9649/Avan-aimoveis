...
export default async function ClientsPage({searchParams}:{searchParams:Promise<Query>}) {
  ...
  return <div className="admin-content"><PageHeader eyebrow="Relacionamento" title="Clientes" description="Consulte contatos, origem e histórico comercial da sua carteira." action={<Link className="admin-button primary" href="/painel/clientes/novo">Novo cliente</Link>}/>
    <ListFilters scope="clientes" userId={user.id} query={query}>
      <details className="advanced-filters client-advanced-filters">
        <summary><SlidersHorizontal aria-hidden="true"/> Filtros</summary>
        <div className="advanced-filters-panel">
          <header><strong>Filtros de clientes</strong><small>Refine a lista sem alterar a busca.</small></header>
          <label>Origem<input name="origin" defaultValue={origin} placeholder="Site, indicação, Instagram…"/></label>
