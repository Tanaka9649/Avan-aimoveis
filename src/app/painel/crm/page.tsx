...
export default async function CrmPage({searchParams}:{searchParams:Promise<Query>}){
 ...
 const cards=dealRows.map(card=>({...card,value:amount(card.value),nextActionAt:card.nextActionAt?.toISOString()??null,stageDays:Math.max(0,Math.floor((referenceTime-card.stageEnteredAt.getTime())/86400000))}));
