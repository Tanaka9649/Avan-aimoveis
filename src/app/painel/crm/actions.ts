...
export async function saveDeal(_:State,form:FormData):Promise<State>{
 ...
 const p=z.object({id:z.union([z.uuid(),z.literal("")]),clientId:z.uuid(),title:z.string().trim().min(3).max(180),stageId:z.uuid(),amount:optionalMoney,nextActionAt:z.string().max(40),lostReason:z.string().trim().max(2000),note:z.string().trim().max(10000),assignedTo:z.union([z.uuid(),z.literal("")])}).safeParse(Object.fromEntries(form));
 ...
 if(v.id){const [existing]=await db.select({id:deals.id}).from(deals).where(and(eq(deals.id,v.id),eq(deals.clientId,v.clientId)));if(!existing)return {ok:false,message:"Oportunidade indisponível."};}
 const id=v.id||randomUUID();const values={clientId:v.clientId,title:v.title,stageId:v.stageId,estimatedValueCents:v.amount,nextActionAt:next,lostReason:stage.isLost?v.lostReason:null,updatedAt:new Date()};
 try{await db.batch([
