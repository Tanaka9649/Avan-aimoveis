...
export async function publicProperties(slug?: string): Promise<Property[]> {
  ...
  const photos=ids.length?await getDb().select({id:propertyPhotos.id,propertyId:propertyPhotos.propertyId,isCover:propertyPhotos.isCover,position:propertyPhotos.position}).from(propertyPhotos).where(inArray(propertyPhotos.propertyId,ids)):[];
  const galleries=new Map<string,typeof photos>();
  for(const photo of photos)galleries.set(photo.propertyId,[...(galleries.get(photo.propertyId)||[]),photo]);
  return rows.map((row) => {const gallery=(galleries.get(row.id)||[]).sort((a,b)=>Number(b.isCover)-Number(a.isCover)||a.position-b.position).map(photo=>`/api/property-photos/${photo.id}`);return { ...row, area: Number(row.area || 0), image: gallery[0]||"/property-placeholder.svg", gallery: gallery.length?gallery:["/property-placeholder.svg"] };});
}
