...
export const clientPropertyPresentations = pgTable("client_property_presentations", { id: uuid("id").defaultRandom().primaryKey(), clientId: uuid("client_id").references(() => clients.id, { onDelete: "cascade" }).notNull(), propertyId: uuid("property_id").references(() => properties.id, { onDelete: "cascade" }).notNull(), userId: uuid("user_id").references(() => users.id, { onDelete: "set null" }), channel: varchar("channel", { length: 40 }).notNull(), presentedAt: timestamp("presented_at", { withTimezone: true }).defaultNow().notNull() }, (t) => [index("client_presentations_idx").on(t.clientId, t.presentedAt), index("property_presentations_idx").on(t.propertyId, t.presentedAt)]);
...
export const proposals = pgTable("proposals", { id: uuid("id").defaultRandom().primaryKey(), dealId: uuid("deal_id").references(() => deals.id).notNull(), propertyId: uuid("property_id").references(() => properties.id), advertisedAmountCents: integer("advertised_amount_cents"), amountCents: integer("amount_cents").notNull(), counterAmountCents: integer("counter_amount_cents"), status: proposalStatus("status").default("enviada").notNull(), validUntil: timestamp("valid_until", { withTimezone: true }), notes: text("notes"), ...audit }, (t) => [index("proposals_property_idx").on(t.propertyId, t.createdAt)]);
